...
shared_data_t *shared = create_shared_memory_area(sizeof(shared_data_t));
        ...
        // Αρχικοποίηση των flags
        for (int i = 0; i < y_chars; i++) {
                shared->lineready[i] = 0;
         }
        ...
        for (int i = 0; i < NPROCS; i++) {
                ...
                if (pid < 0) {
                        ...
                        for (int line = i; line < y_chars; line += NPROCS) {
                                compute_mandel_line(line, shared->shared_buffer[line]);

                                shared->lineready[line] = 1;
                        }
                        exit(0);
                }
        }
        ...
        for(int line=0;line<y_chars;line++){
                while(shared->lineready[line]==0){
                        usleep(1000);
                }
                        output_mandel_line(1, shared->shared_buffer[line]);
        }
